...
def dt_univar_plots(data, var, target=None, bins="auto"):
  ...
  def numerical_analysis(dataframe, column_name, cat_col=None, bins='auto', log_scale=False ):
    # create the figure
    ...
    # plot the kdeplot
    sns.kdeplot(data=dataframe, x=column_name, hue=cat_col,log_scale=log_scale, ax=ax1)
    # plot the boxplot
    sns.boxplot(data=dataframe, x=column_name, hue=cat_col,log_scale=log_scale, ax=ax2)
    # plot the histplot
    sns.histplot(data=dataframe, x=column_name,bins=bins,kde=True, hue=cat_col,log_scale=log_scale, ax=ax3)
    plt.tight_layout()
    plt.show()
...
def numerical_categorical_analysis(dataframe, cat_column_1, num_column):
    ...
    ax1[1].set_xticklabels(ax1[0].get_xticklabels(), rotation=45)
...
def numerical_numerical_col_analysis(dataframe, num_col_1, num_col_2, cat_col=None):
    fig = plt.figure(figsize=(15,10))
    # generate the layout
    grid = GridSpec(nrows=2, ncols=2, figure=fig)
    # set subplots
    ...
    sns.scatterplot(data=dataframe, x=num_col_1, y=num_col_2,hue=cat_col, ax=ax1)
    
    # plot the 2d histplot
    sns.histplot(data=dataframe, x=num_col_1, y=num_col_2, hue=cat_col,ax=ax2)
    
    # plot the 2d kde
    # sns.kdeplot(data=dataframe, x=num_col_1, y=num_col_2, hue=cat_col,ax=ax3)
...
def multivariate_analysis(dataframe, num_column, cat_column_1, cat_column_2):
    ...
    # Bar plot
    sns.barplot(data=dataframe, x=cat_column_1, y=num_column, hue=cat_column_2, ax=ax[0])

    # Box plot
    sns.boxplot(data=dataframe, x=cat_column_1, y=num_column, hue=cat_column_2, ax=ax[1])

    # Violin plot
    sns.violinplot(data=dataframe, x=cat_column_1, y=num_column, hue=cat_column_2, ax=ax[2])

    # Strip plot
    sns.stripplot(data=dataframe, x=cat_column_1, y=num_column, hue=cat_column_2, dodge=True, ax=ax[3])

    # Rotate x-tick labels for all plots
    for a in ax:
...
#     data = (
#         dataframe.loc[:,[col1,col2]].dropna()
#     )
...
#     data = (
#         dataframe.loc[:, [num_col, cat_col]].dropna()
#     )
